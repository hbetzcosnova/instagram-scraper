...
const scrapePost = (request, itemSpec, entryData) => {
    ...
    return {
        ...
        commentsCount:      (item.edge_media_to_comment && item.edge_media_to_comment.count) || 0,
        likesCount:         (item.edge_liked_by && item.edge_liked_by.count) || 0,
        prevLikesCount:     (item.edge_media_preview_like && item.edge_media_preview_like.count ) || 0,
        text:               item.edge_media_to_caption.edges[0] && item.node.edge_media_to_caption.edges[0].node.text,
        // misc data
...
const scrapePosts = async (page, request, itemSpec, entryData, requestQueue) => {
    ...
    const output = posts[itemSpec.id].map(item => ({
        ...
        commentsCount:      (item.node.edge_media_to_comment &&     item.node.edge_media_to_comment.count) || 0,
        likesCount:         (item.node.edge_liked_by &&             item.node.edge_liked_by.count) || 0,
        prevLikesCount:     (item.node.edge_media_preview_like &&   item.node.edge_media_preview_like.count ) || 0,
